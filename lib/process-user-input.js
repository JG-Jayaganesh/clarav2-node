...
const processPayload = function(error, payload){
  let conversationPayload = payload.payload;
  const callback = payload.callback;
  const requiredData = conversationPayload.context.dataParam;
  let data = payload.resultSet;
    ...
    switch (requiredData) {
      ...
      //console.log(data);
        if(data != undefined && data.length>0){
          ...
          data = data[0];
          clarav2Session.setVal(payload.request.session, 'customer_info', data);
      ...
      case 'recall':
        ...
        clarav2Session.setVal(payload.request.session, 'vehicle_info', data);
        callback(error, payload);
        break;
      default:
        callback(error, payload);
...
const processUserInput = {
    init: function(payload, callback){
      ...
      let requiredData = conversationPayload.context.dataParam;
      if(requiredData == undefined){
        requiredData = conversationPayload.context.conversationEndFlag;
    ...
    getEntityDefinitions: function(entity){
      switch (entity){
        case 'customer':
          return ['vehicle_no', 'payment_type', 'model_id', 'class_id', 'city', 'marital_sts', 'first_name', 'last_name', 'no_of_child'];
        case 'car_model':
          return ['model', 'price'];
        case 'check_recall_model':
          return ['model_type', 'to', 'from'];
        case 'suggest_model_selection':
          return ['Age', 'MaritalStatus', 'budget', 'city', 'fuelType', 'seatCapacity'];
      }
    },
