  ...
  function postConversationMessage(text) {
    ...
    http.onload = function() {
      if (http.status === 200 && http.responseText) {
        ...
        ConvApi.setWatsonPayload(response);
      } else {
        ConvApi.setWatsonPayload({output: {text: [
...
var TooltipDialogs = (function() {
  ...
  function closeDialogs() {
    ...
    ConvApi.initConversation(); // Load initial Watson greeting after overlays are gone.
    hideOverlays();
...
var Conversation = (function() {
  ...
  function chatSetup() {
    ...
    var currentRequestPayloadSetter = ConvApi.setUserPayload;
    ConvApi.setUserPayload = function(payload) {
      currentRequestPayloadSetter.call(ConvApi, payload);
      displayMessage(payload, authorTypes.user);
    };

    var currentResponsePayloadSetter = ConvApi.setWatsonPayload;
    ConvApi.setWatsonPayload = function(payload) {
      currentResponsePayloadSetter.call(ConvApi, payload);
  ...
  function sendMessage(newText) {
    ...
    ConvApi.postConversationMessage(text);
  }
