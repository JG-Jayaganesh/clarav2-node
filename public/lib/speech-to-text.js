...
var STTModule = (function () {
  ...
  function micON() { // When the microphone button is clicked
    // if (recording === false) {
    //   if (records === 0) {
    //      // The first time the mic is clicked - inform user
    //     ConvApi.setWatsonPayload({output: {text: ['Accept the microphone prompt in your browser. Watson will listen soon.'], ref: 'STT'}}); // Dialog box output to let the user know we're recording
    //     records++;
    //   } else {
    //     ConvApi.setWatsonPayload({output: {ref: 'STT'}}); // Let the user record right away
    //   }
    // } else {
    //   recording = false;
    //   try {
  ...
  function speechToText() {
    // Set CSS class of mic to indicate that we're currently listening to user input
    textdata.setAttribute('class', 'inactive-text');
    ...
    fetch('/api/speech-to-text/token')        // Fetch authorization token for Watson Speech-To-Text
      ...
      .then(function (token) {                 // Pass token to Watson Speech-To-Text service
        ...
        stream.promise()                                // Once all data has been processed...
          .then(function (data) {
            textdata.setAttribute('class', 'active-text');                      // ...put all of it into a single array
            mic.setAttribute('class', 'inactive-mic');  // Reset our microphone button to visually indicate we aren't listening to user anymore
            recording = false;
            console.log(data);                       // We aren't recording anymore
            if (data.length !== 0) {                    // If data is not empty (the user said something)
              var dialogue = data.pop().results[0];                // Get the last data variable from the data array, which will be the finalized Speech-To-Text transcript
              if (dialogue != undefined && (dialogue.alternatives) && (dialogue.alternatives[0].transcript !== '') && (dialogue.final === true)) { // Another check to verify that the transcript is not empty and that this is the final dialog
                ConversationPanel.STTSubmit();
              ...
              // ConvApi.setWatsonPayload({output: {text: ['Microphone input cancelled. Please press the button to speak to Watson again']}}); // If the user clicked the microphone button again to cancel current input
            }
          })
          .catch(function (err) { // Catch any errors made during the promise
            ...
            mic.setAttribute('class', 'inactive-mic'); // Reset our microphone button to visually indicate we aren't listening to user anymore
            // ConvApi.setWatsonPayload({output: {text: ['Watson timed out after a few seconds of inactivity. Please press the button to speak to Watson again.']}});
          });
      })
